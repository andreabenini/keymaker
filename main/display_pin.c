...
static void number_btn_event_cb(lv_event_t *e)
{
    ...
    ESP_LOGI(TAG, "Number button %d clicked (portrait=%d, width=%d, height=%d)",
             digit, g_is_portrait, g_current_width, g_current_height);
...
static void hamburger_btn_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "Hamburger button clicked - toggling rotation from %s to %s",
                 g_is_portrait ? "portrait" : "landscape",
                 g_is_portrait ? "landscape" : "portrait");
        ...
        if (g_is_portrait) {
            ...
            // (Touch calibration was done in landscape, so keep those settings)
        } else {
